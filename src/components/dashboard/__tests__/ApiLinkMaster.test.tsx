...
vi.mock('@/hooks/useProfiles', () => ({ useProfiles: () => ({ data: [{ id: '1', full_name: 'Alice', erp_username: 'ERP1', api_username: '111' }] }), useProfilesHasApiUsername: () => ({ data: true }) }));
...
describe('ApiLinkMaster', () => {
  ...
  it('calls sync endpoint when clicking Save (existing mapping present)', async () => {
    ...
    vi.resetModules();
    vi.doMock('@/hooks/useProfiles', () => ({ useProfiles: () => ({ data: [{ id: '1', full_name: 'Alice', erp_username: 'ERP1', api_username: '111' }] }), useProfilesHasApiUsername: () => ({ data: true }) }));
    vi.doMock('@/hooks/useLinkMaster', () => ({
  ...
  it('disables Save when api_username column missing and shows warning', async () => {
    vi.resetModules();
    vi.doMock('@/hooks/useProfiles', () => ({ useProfiles: () => ({ data: [{ id: '1', full_name: 'Alice', erp_username: 'ERP1' }] }), useProfilesHasApiUsername: () => ({ data: false }) }));
    vi.doMock('@/hooks/useLinkMaster', () => ({
      useLinkMasters: () => ({ data: [] }),
      useLinkMasterByERP: (erp?: string) => ({ data: null }),
      useCreateLinkMaster: () => mockCreate,
    ...
    expect(screen.getByText("`api_username` column missing in `profiles" , { exact: false })).toBeInTheDocument();
  });
